...
parser.add_argument('-i', '--input', type=str, default='data/iwp/iwp_test.json', help='Input label file.')
parser.add_argument('-c', '--clip', type=bool, default=False, help='Whether to use CLIP model.')
parser.add_argument('--prompt', type=str, default='ice wedge polygon', help='Prompt for CLIP model.')
parser.add_argument('--n_candidates', type=int, default=500, help='Maximum number of candidates to be selected from SAM.')
...
def filter_masks(
    ...
    stability_score_threshold: float = .95,
    query: str = '',
...
def prediction(**args):
    ...
    for idx, data in enumerate(dataset):
        ...
        image = cv2.imread(data['file_name'], cv2.IMREAD_COLOR)
        ...
        for mask in masks:
            pred_sam = {}
            pred_sam['image_id'] = data["image_id"]
        ...
        sam_image.save('{}/{}.png'.format(sam_dir, data['file_name'].split('/')[-1].split('.')[0]))
        ...
        pred_image.save('{}/{}.png'.format(pred_dir, data['file_name'].split('/')[-1].split('.')[0]))
...
def generate_evaluation_metric(**args):
    ...
    dataset = json.load(open('data/{}/{}_test.json'.format(args['dataset'], args['dataset'])))
    coco_gt = {}
    coco_gt['images'] = []
    coco_gt['annotations'] = []
    ann_id = 0
    ...
    for data in dataset:
        ...
        for ann in data['annotations']:
            ...
            ann_coco['id'] = ann_id
            ann_id += 1
            ann_coco['image_id'] = data['image_id']
            ann_coco['category_id'] = ann['category_id']
            ann_coco['segmentation'] = ann['segmentation']
            ann_coco['bbox'] = ann['bbox']
            ann_coco['area'] = (ann['bbox'][2] - ann['bbox'][0]) * (ann['bbox'][3] - ann['bbox'][1])
            ann_coco['iscrowd'] = 0
...
prediction(input_file=LABEL_FILE, output_dir='results', dataset='iwp')
generate_evaluation_metric(dataset='iwp', num_classes=1)
